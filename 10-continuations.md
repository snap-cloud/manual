...
can be represented as a ringed script.
...
The continuation of a command block, as we’ve seen, is a simple script with
...
For example, in the expression {inline alt="image884.png" width="1.3125in"}`images/10-continuations/image884.png` Snap<em>!</em>
...
people all waiting for each other, with CPS
what happens is that each little person hands off the
...
versions of all the reporters, add and so on. Snap<em>!</em> provides a
...
::::{grid} 2
:::{grid-item}
{img alt="image913.png" width="4.03819in"}`images/10-continuations/image913.png`
:::
:::{grid-item}
{img alt="image914.png" width="3.94514in"}`images/10-continuations/image914.png`
:::
::::
...
First class continuations are an experimental feature in Snap<em>!</em> and
...
that can be used inside a looping construct such
...
Snap<em>!</em> can be running several scripts at once, within a single sprite
...
any moment, but Snap<em>!</em> switches its attention from one script to
...
command (a *thread* is the technical term for the
